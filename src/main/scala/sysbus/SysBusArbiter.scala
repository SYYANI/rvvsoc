...
class SysBusArbiter(mastersN : Int) extends Module {
    ...
    val master_vdat_i = Wire(Vec(mastersN, UInt(128.W)))
    ...
    val master_ven_i = Wire(Vec(mastersN,Bool()))

    for(i <- 0 until mastersN){
        master_vdat_i(i) := io.in(i).vec_data_i
        ...
        master_ven_i(i) := io.in(i).vec_en_i
    }
    ...
    for(i <- 0 until mastersN){
        io.in(i).vec_data_o := Mux(prev_turn(i),io.out.vec_data_o, 0.U)
    ...
    io.out.vec_data_i := master_vdat_i(turnNum)
    ...
    io.out.vec_en_i := master_ven_i(turnNum)
}
