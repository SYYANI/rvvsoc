...
import rvvsoc.core.ALU.{ALU_OP_ALS, ALU_OP_VAD, ALU_OP_VML}
import rvvsoc.mmu._
import rvvsoc.utils.{Debug, Info}
...
class Datapath(CoreID: Int) extends Module {
    ...
    val vec_regfile = Mem(8,UInt(128.W))
    val vcsr_valid = RegInit(Bool(), false.B)
    val vreg_waddr = RegInit(UInt(5.W), 0.U)

    val vaddvv = WireDefault(0.U(128.W))
    val vmulvv = WireDefault(0.U(128.W))
    ...
    val dmem_vector_reg = RegInit(UInt(), 0.U(128.W))
    ...
    // ---------- NPC ----------
//    val pc = RegInit((0xffffff80 - 4).S(32.W).asUInt) // initial pc
    ...
    //vector prefetch
    val vs1_data = Mux(ex_ctrl_sigs.alu_op === ALU_OP_VAD || ex_ctrl_sigs.alu_op === ALU_OP_VML, vec_regfile(ex_reg_inst(19, 15)), 0.U)
    val vs2_data = Mux(ex_ctrl_sigs.alu_op === ALU_OP_VAD || ex_ctrl_sigs.alu_op === ALU_OP_VML, vec_regfile(ex_reg_inst(24, 20)), 0.U)
    when(ex_ctrl_sigs.alu_op === ALU_OP_VAD) {
        val num = 128 / 4
        vaddvv := Cat(Seq.tabulate(num)(
            n => (vs1_data(4 * (n + 1) - 1, 4 * n) + vs2_data(4 * (n + 1) - 1, 4 * n))
        ).reverse)
        vec_regfile(ex_reg_inst(11, 7)) := vaddvv
    }
    when(ex_ctrl_sigs.alu_op === ALU_OP_VML) {
        val num = 128 / 16
//        vmulvv := Cat(Seq.tabulate(num)(
//            n => (vs1_data(16 * (n + 1) - 1, 16 * n) * vs2_data(16 * (n + 1) - 1, 16 * n))
//        ).reverse)
        vmulvv := Cat((for (n <- 0 until num if n % 2 == 0) yield
            (vs1_data(16 * (n + 1) - 1, 16 * n) * vs2_data(16 * (n + 1) - 1, 16 * n))
          ).reverse)
//        vmulvv := Cat((for (n <- 0 until num if n % 2 == 0) yield {
//            val vs1 = vs1_data(16 * (n + 1) - 1, 16 * n)
//            val vs2 = vs2_data(16 * (n + 1) - 1, 16 * n)
//            printf("n = %d, vs1 = %d, vs2 = %d\n", n.U, vs1, vs2)
//            vs1 * vs2
//        }).reverse)
//        Info("vmulvv: %x\n", vmulvv)
        vec_regfile(ex_reg_inst(11, 7)) := vmulvv
    }
    ...
    io.dmem.req.vec_en := ex_functioning && ex_ctrl_sigs.mem && (isWrite(ex_ctrl_sigs.mem_cmd) || isRead(ex_ctrl_sigs.mem_cmd)) && ex_ctrl_sigs.alu_op === ALU_OP_ALS

    io.dmem.req.vwr_data := vec_regfile(ex_reg_inst(11, 7))
    ...
    when (!wb_replay && mem_functioning) {
        ...
        dmem_vector_reg := io.dmem.res.vrd_data
    }
    ...
    when(wb_functioning && wb_ctrl_sigs.alu_op===ALU_OP_ALS) {
        vcsr_valid := true.B
        vreg_waddr := wb_reg_inst(11,7)
        val v_wb_addr = wb_reg_inst(11,7)
        vec_regfile(v_wb_addr) := dmem_vector_reg
    }
...
//      "mem_back_addrr: %x\n" +
//      "ex_back_addr: %x\n" +
//      "io.dmem.req.vec_en: %x",pc, io.imem.inst,wb_waddr,mem_waddr,ex_waddr,io.dmem.req.vec_en)
    ...
    Info("inst[%x] -> %x\n" +
      ...
      "vec_regfile(7.U) : %x\n",
        pc, io.imem.inst,
        vec_regfile(0.U), vec_regfile(1.U), vec_regfile(2.U), vec_regfile(3.U),
        vec_regfile(4.U), vec_regfile(5.U), vec_regfile(6.U), vec_regfile(7.U))
    // debug info below
}