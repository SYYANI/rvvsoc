...
class SysBusConnector extends Module {
    ...
    io.req.vec_data_wr := Mux(io.dmem.req.wen, io.dmem.req.vec_data_wr, 0.U(128.W))
    io.req.sel := Mux(dmem_en, io.dmem.req.sel,
        Mux(imem_en, io.imem.req.sel, 0.U(32.W)))
    io.req.wen := io.dmem.req.wen
    io.req.vec_en := io.dmem.req.ven
    ...
    io.imem.res.vec_data_rd := 0.U(128.W)

    io.dmem.res.vec_data_rd := 0.U(128.W)
    ...
    when (dmem_reg_en) {
        io.dmem.res.vec_data_rd := io.res.vec_data_rd
