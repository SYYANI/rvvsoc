...
trait InstructionPatterns {
  ...
  def VSETVLI = BitPat("b?????????????????111?????1010111")
  def VLE = BitPat("b000000100000?????????????0000111")
  def VSE = BitPat("b000000100000?????????????0100111")
  def VADDVV = BitPat("b0000001??????????000?????1010111")
  def VMULVV = BitPat("b100101???????????010?????1010111")
}