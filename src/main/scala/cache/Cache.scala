...
import rvvsoc.utils.{Debug, Warn}
...
class Cache(blockWidth : Int, wayCount : Int, indexWidth : Int, notToCache: Seq[(BitPat, Bool)]) extends Module{
    ...
    val r_tags = RegInit(VecInit(Seq.fill(wayCount * (1 << indexWidth))(0.U((32 - indexWidth - blockWidth).W))))
    val r_valids = RegInit(VecInit(Seq.fill(wayCount * (1 << indexWidth))(false.B)))
    val r_dirtys = RegInit(VecInit(Seq.fill(wayCount * (1 << indexWidth))(false.B)))
    val r_blocks = RegInit(VecInit(Seq.fill(wayCount * (1 << (indexWidth + blockWidth)))(0.U(8.W))))
    ...
    val vdat = io.bus.slave.vec_data_i
    ...
    val cur_vdat = RegInit(UInt(128.W), 0.U)
    val cur_sel = RegInit(UInt(4.W), 0.U)
    val cur_we = RegInit(Bool(), false.B)
    val cur_ven = RegInit(Bool(), false.B)
    ...
    val bus_in_vdat = io.bus.master.vec_data_o
    ...
    def directlyAccessBus(_adr : UInt, _dat : UInt, _we : Bool, _sel : UInt, _vdat : UInt, _vwe : Bool){
        cyc_o := true.B
        we_o := _we
        ven_o := _vwe
        sel_o := _sel
    ...
    when(cyc && stb && state === STATE_IDLE){
        when(!entry_valid){
            ...
            cur_we := we
            when(direct_access){
                directlyAccessBus(adr, dat, we, sel, vdat, ven)
        ...
        when(op_counter === 0.U){
            directlyAccessBus(cur_adr, cur_dat, cur_we, cur_sel, cur_vdat, cur_ven)
            op_counter := next_counter
        }.otherwise{
            endBusTransaction()
            when(bus_ack){
